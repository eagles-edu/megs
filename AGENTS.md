...
- Dev-only plaintext dictionaries live in `dev/<title>.ext` (generated by `tools/fnv1a64-convert.mjs --round-trip --title <title>`). `title` must be set in the page answer-key JSON; devtools fetches `../dev/<title>.ext` (or inline `data-exercise-dev-dict`) for autofill only.
