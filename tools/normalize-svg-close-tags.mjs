#!/usr/bin/env node
/**
 * Bulk-normalize split SVG closing tags in HTML/HTM files.
 *
 * Converts:
 *   </svg
 *     >
 ...
 *
 * Usage:
 *   node tools/normalize-svg-close-tags.mjs
 *   node tools/normalize-svg-close-tags.mjs --target lesson-1-nouns --dry-run
 */
...
function fail(message) {
  console.error(`normalize-svg-close-tags: ${message}`)
  process.exit(1)
}
...
function printUsage() {
  console.log(`Usage:
  node tools/normalize-svg-close-tags.mjs [options]
...
function normalizeSplitSvgCloseTags(source) {
  let replacementCount = 0
  const output = String(source || "").replace(/<\/svg[ \t]*\r?\n[ \t]*>/gi, () => {
    replacementCount += 1
    return "</svg>"
  })
...
function main() {
  ...
  for (const absolutePath of targets) {
    ...
    const { output, replacementCount } = normalizeSplitSvgCloseTags(input)
    if (output === input) continue

    changedFiles += 1
    normalizedTags += replacementCount
    if (options.write) {
    ...
    console.log(`[${mode}] ${relativePath}: normalized-svg-close-tags:${replacementCount}`)
  }

  const modeText = options.write ? "applied" : "would change"
  console.log(
    `\nSummary: scanned ${targets.length} file(s), ${changedFiles} ${modeText}, ` +
      `${writtenFiles} written, ${normalizedTags} split svg close tags normalized.`
  )
}
