...
const resolveTableWrapBundleSrc = () => {
  const scripts = document.getElementsByTagName("script")
  for (let i = scripts.length - 1; i >= 0; i--) {
    const srcAttr = scripts[i].getAttribute("src") || ""
    if (!srcAttr) continue
    if (srcAttr.indexOf("main.bundle.js") === -1) continue
    const absolute = scripts[i].src || srcAttr
    return absolute.replace(/main\.bundle\.js(?:\?.*)?$/, "tablewrap.bundle.js")
  }
  return ""
}

const resolveTableWrapCssSrc = () => {
  const bundleSrc = resolveTableWrapBundleSrc()
  if (!bundleSrc) return ""
  return bundleSrc.replace(/\/js\/tablewrap\.bundle\.js(?:\?.*)?$/, "/css/tablewrap.css")
}

const hasTableWrapCandidates = () => {
  const scope = document.querySelector("main#content, #content") || document
  const tables = scope.querySelectorAll("table")
  return Boolean(tables && tables.length)
}

const ensureTableWrapStylesheet = (done) => {
  const callback = typeof done === "function" ? done : () => {}
  const href = resolveTableWrapCssSrc()
  if (!href) {
    callback()
    return
  }
  ...
  let existing = document.querySelector('link[data-tablewrap-styles="true"]')
  if (existing) {
    callback()
    return
  }
...
const loadTableWrapBundle = () => {
  if (window.__tableWrapBundleLoading || window.__tableWrapBundleLoaded) return
  const src = resolveTableWrapBundleSrc()
  if (!src) return

  window.__tableWrapBundleLoading = true
  const loadScript = () => {
    const script = document.createElement("script")
    script.type = "module"
    script.src = src
    ...
    script.onload = () => {
      window.__tableWrapBundleLoaded = true
      window.__tableWrapBundleLoading = false
    }
    script.onerror = () => {
      window.__tableWrapBundleLoading = false
    }
  ...
  ensureTableWrapStylesheet(loadScript)
}

const scheduleTableWrapBundle = () => {
  const start = () => {
