...
const matches = (el, selector) => {
  ...
  const scope = el.parentElement || el.parentNode || (typeof document !== 'undefined' ? document : null);
...
const QAAccordion = {
  init() {
    ...
    const handleToggleClick = (ev, explicitToggle) => {
      const toggle = explicitToggle || (ev && ev.target ? closest(ev.target, 'a.accordion-toggle') : null);
