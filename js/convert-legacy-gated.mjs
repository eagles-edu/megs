...
function createAnswerKey(scraped, answerFieldCount) {
  const answerArray = {};
  scraped.questions.forEach((question, index) => {
    ...
    answerArray[key] = {
      ...
      answersAccepted: Array.from({ length: 1 }, () => Array.from({ length: answerFieldCount }, () => ''))
    };
  });
...
function injectTemplate(templateHtml, scraped, answerFieldCount, testMode, targetPath) {
  ...
  scraped.questions.forEach((question) => {
    const questionDom = buildQuestionDom($, question, answerFieldCount || question.answerFieldCount, testMode, fileName);
    insertTarget.before(questionDom);
  });

  const answerKey = createAnswerKey(scraped, answerFieldCount);
...
function validateCounts(scraped, expectedQuestions, answerKey) {
...
function main() {
  ...
  parser.add_argument('--answer-fields', {
    ...
    default: 4,
    dest: 'answerFields',
    help: 'Number of answer input fields to generate per question'
  });
  ...
  validateCounts(scraped, args.questions, answerKey);
