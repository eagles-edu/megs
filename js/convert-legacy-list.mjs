...
function syncFlyoutCurrentState($, targetPath, headline) {
  const menu = $("ul.flyout-menu").first()
  if (!menu.length) return { matched: false, strategy: "none" }
  ...
  const normalizedHeadline = (headline || "").replace(/\s+/g, " ").trim().toLowerCase()
...
const CONVERTED_CHECKS = [
  ...
  /<ul[^>]*class=["'][^"']*\bflyout-menu\b[^"']*["']/i,
]

function isListConverted(html) {
  if (!html) return false
  return CONVERTED_CHECKS.every((regex) => regex.test(html))
}
...
const SHELL_VERIFY_CHECKS = [
  ...
  { name: "flyout menu markup", pattern: /<ul[^>]*class=["'][^"']*\bflyout-menu\b[^"']*["']/i },
]

function verifyListShellContent(html) {
...
function runSingleConversion(args) {
  ...
  const verification = verifyListShellContent(conversion.updated)
  if (!verification.ok) {
    console.log(`[verify] FAIL :: ${verification.missing.join(", ")}`)
    process.exitCode = 1
    return
  }
        ...
        try {
          ...
          if (args.diffPreview) {
            ...
            if (!args.verifyShell) {
              ...
              if (!verification.ok) {
                verifyFailures += 1
                const verifyMessage = verification.missing.join(", ")
                touchedEntries.push({
                  file: shortName,
                  status: "pass",
                  verifyStatus: "fail",
